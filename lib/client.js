window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* @param recentlyDone - monitor-tracked running鈫抜dle transition still within
		...
		*/
		function sessionTabState(summary, hasPendingInteraction = false, recentlyDone = false) {
		...
		* @param recentlyDone - ids of sessions whose running鈫抜dle transition is
		* still within the monitor's visibility window (shown green).
		* @returns per-state counts (never partial: a fresh object per call).
		*/
		function aggregateTabCounts(byId, pendingIds, recentlyDone) {
			const counts = {
				running: 0,
				pending: 0,
				done: 0
			};
			for (const summary of Object.values(byId)) switch (sessionTabState(summary, pendingIds?.has(summary.id) === true, recentlyDone?.has(summary.id) === true)) {
				case "running":
					counts.running += 1;
					break;
				case "pending":
		...
		*/
		...
		const TAU = Math.PI * 2;
		/**
		* Fixed trailing gap in radians 鈥?at least 30掳 so the anchor reads clearly at
		...
		* ring is complete 鈥?the gap exists to show rotation, and a static done or
		...
		* closing chord 鈥?the nonzero winding rule fills exactly the ring sector,
		...
		* (`ReadonlyMap<SessionId, 鈥?`); its keys are the sessions waiting on the user.
		...
		*/
		function createTabStatusMonitor(doc, list, pendingInteractions, renderer, options = {}) {
			...
			/** Sessions whose running鈫抜dle transition is still shown green: id 鈫?expiry. */
			...
			/**
			* Full re-evaluation: detect running鈫抜dle transitions, prune expired done
			...
			*/
			const evaluate = () => {
				...
				for (const id of previousRunning) {
					...
					if (byId[id] !== void 0 && !pendingIds.has(id)) doneUntil.set(id, now + doneVisibleMs);
				}
				previousRunning = runningIds;
				for (const [id, until] of doneUntil) if (until <= now) doneUntil.delete(id);
				counts = aggregateTabCounts(byId, pendingIds, doneUntil.size > 0 && runningIds.size === 0 ? new Set(doneUntil.keys()) : void 0);
		...
		*/
		function apply(ctx, config) {
			...
			ctx.effect(() => {
				...
				const monitor = createTabStatusMonitor(document, ctx.sessions.list, ctx.uiSession.pendingInteractions, renderer, {
