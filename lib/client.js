window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function sessionTabState(status, recentlyDone = false) {
		...
		*/
		function aggregateTabCounts(statuses, recentlyDone) {
			const counts = {
				running: 0,
				pending: 0,
				done: 0
			};
			for (const [id, status] of statuses) switch (sessionTabState(status, recentlyDone?.has(id) === true)) {
				case "running":
					counts.running += 1;
					break;
				case "pending":
		...
		*/
		function createTabStatusMonitor(doc, sessionStatus, renderer, options = {}) {
			...
			*/
			const evaluate = () => {
				...
				for (const [id, status] of statuses) if (status.running === true) runningIds.add(id);
				for (const id of previousRunning) {
					if (runningIds.has(id)) continue;
					const status = statuses.get(id);
					if (status !== void 0 && status.pendingInteraction === void 0) doneUntil.set(id, now + doneVisibleMs);
				}
				previousRunning = runningIds;
				for (const [id, until] of doneUntil) if (until <= now) doneUntil.delete(id);
				counts = aggregateTabCounts(statuses, doneUntil.size > 0 && runningIds.size === 0 ? new Set(doneUntil.keys()) : void 0);
		...
		*/
		function apply(ctx, config) {
			...
			ctx.effect(() => {
				...
				const monitor = createTabStatusMonitor(document, ctx.uiSession.sessionStatus, renderer, {
